...
The dashboard also tracks a lightweight hiring pipeline per candidate — a category (Shortlisted / Flagged for Risk / Not Suitable / Not Moving Forward) that defaults automatically: high fraud risk routes to Flagged for Risk, and missing even one of the JD's must-have requirements (not a nice-to-have — cross-referenced against the JD's actual `must_haves` list, not just whether the JD-fit agent's `requirements_missing` field is non-empty) routes to Not Suitable regardless of how high the overall fit score is, since a must-have is a hard gate, not one input among many into an average. Everything else defaults to Shortlisted. Any category is movable by a human from any list view or the profile page. and a round (AI Screening → Technical Screening, extensible). Every move or advance is timestamped, and a comment is required unless the system already supplied its own reason for that move (e.g. moving a high-fraud-risk candidate to Flagged for Risk just confirms the agent's own finding). History is kept per candidate — this is also the raw input for a confidence-calibration feedback loop (see architecture doc, Section 4.16): comparing what a human actually decided against the agent's stated confidence, over time. Rejected candidates move to a collapsed "Not Moving Forward" list so the active view stays short.
